...
class OhosInitMgr {
public:
    static OhosInitMgr &GetInstance();

    void Register(InitCall initFunc);

    void RegisterFeature(InitCall initFunc);
...
private:
    ...
    std::mutex initFuncListLock_;
    std::vector<InitCall> initFuncList_;
    std::vector<InitCall> initFeatureFuncList_;
    bool isInit { false };
};

#define SYSEX_SERVICE_INIT(func)                    \
namespace {                                         \
class ServiceInitcall_##func {                      \
public:                                             \
    ServiceInitcall_##func()                        \
    {                                               \
        OhosInitMgr::GetInstance().Register(func);  \
    }                                               \
};                                                  \
ServiceInitcall_##func serviceInitcall_##func;      \
}
...
#define SYSEX_FEATURE_INIT(func)                            \
namespace {                                                 \
class FeatureInitcall_##func {                              \
public:                                                     \
    FeatureInitcall_##func()                                \
    {                                                       \
        OhosInitMgr::GetInstance().RegisterFeature(func);   \
    }                                                       \
};                                                          \
FeatureInitcall_##func serviceInitcall_##func;              \
}
