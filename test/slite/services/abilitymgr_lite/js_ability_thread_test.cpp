...
TEST_F(JsAbilityThreadTest, JsAbilityThreadDefaultTest)
{
    ...
    ASSERT_EQ(jsAbilityThread->GetMessageQueueId(), nullptr);
    ASSERT_EQ(jsAbilityThread->GetAppTaskId(), 0u);
...
TEST_F(JsAbilityThreadTest, JsAbilityThreadNoReleaseTest)
{
    ...
    osMessageQueueId_t messageQueueId = jsAbilityThread->GetMessageQueueId();
